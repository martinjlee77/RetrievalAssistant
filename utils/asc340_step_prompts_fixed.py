"""
Enhanced ASC 340-40 step prompts with IAC framework alignment.
Fixed version with aligned schemas and instructions.
"""
...
class ASC340StepPrompts:
    ...
    def get_step_info() -> dict:
        """
        Metadata for each ASC 340-40 analysis step.
        """
        return {
...
**FAILURE TO FOLLOW:** Any response that doesn't quote specific contract terms or uses generic examples instead of actual document content will be rejected."""
    ...
    def _get_step_instructions(step_number: int) -> str:
        """Get step-specific analysis instructions with IAC framework"""
        instructions = {
...
**ENFORCEMENT:** If you provide generic analysis without quoting actual contract terms, the analysis will be deemed inadequate.""",
...
**ENFORCEMENT:** Analysis must reference actual commission rates, payment conditions, and eligibility criteria from the contract.""",
...
**ENFORCEMENT:** Analysis must reference actual contract durations, renewal terms, and commission structures from the document.""",
...
**ENFORCEMENT:** Journal entries must reflect the actual commission rates, payment triggers, and business model described in the contract."""
        }
        return instructions.get(step_number, "")

    @staticmethod
    def _get_schema_for_step(step_number: int) -> str:
        """Get JSON schema for each step - IAC Framework Aligned"""
        schemas = {
            1: """{
  "step1_analysis": {
  ...
  "evidence_quotes": [
    "Direct quote from the source document... (Source: FY2024 Sales Commission Plan)"
            ...
            2: """{
  "step2_analysis": {
    "cost_classification_points": [
      {
        "topic": "Cost type from the document (e.g., 'Sales Commission at 5% of TCV')",
        "issue": "The issue is to classify this specific cost under ASC 340-40 framework.",
        "analysis": "A detailed analysis applying ASC 340-40-25 tests to the ACTUAL contract terms. This MUST quote specific rates, triggers, and conditions from the contract.",
        "conclusion": "A definitive classification determination (e.g., 'Therefore, this 5% commission qualifies as an incremental cost...')"
      }
    ],
    "overall_conclusion": "A final summary of the cost classification policy framework."
  },
  "evidence_quotes": [
    "Direct quote from the source document... (Source: FY2024 Sales Commission Plan)"
  ]
            ...
            3: """{
  "step3_analysis": {
    "measurement_policy_points": [
      {
        "topic": "Measurement aspect from the document (e.g., 'Commission Calculation Method')",
        "issue": "The issue is to establish measurement and amortization policy for this specific cost structure.",
        "analysis": "A detailed analysis applying ASC 340-40-35 requirements to the ACTUAL contract terms. This MUST quote specific contract durations, renewal terms, and commission structures.",
        "conclusion": "A definitive policy determination (e.g., 'Therefore, commissions will be amortized over...')"
      }
    ],
    "overall_conclusion": "A final summary of the measurement and amortization policy framework."
  },
  "evidence_quotes": [
    "Direct quote from the source document... (Source: FY2024 Sales Commission Plan)"
  ]
  ...
  "step4_analysis": {
    "financial_impact_points": [
      {
        "topic": "Financial impact scenario from the document (e.g., '$50,000 sale with 5% commission')",
        "issue": "The issue is to demonstrate the financial impact using actual contract terms.",
        "analysis": "A detailed analysis showing journal entries and financial statement impact using the ACTUAL commission rates and structures from the contract. This MUST use specific percentages and terms.",
        "conclusion": "A definitive illustration of financial impact (e.g., 'Therefore, a $50,000 sale would result in...')"
      }
    ],
    "overall_conclusion": "A final summary of financial impact and implementation approach."
  },
  "evidence_quotes": [
    "Direct quote from the source document... (Source: FY2024 Sales Commission Plan)"
  ]
