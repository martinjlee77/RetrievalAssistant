...
def _preprocess_markdown_for_html(memo_markdown: str) -> str:
    ...
    # Dynamic ASC 340-40 header support - imports header from configuration
    try:
        from utils.asc340_step_prompts import ASC340StepPrompts
        asc340_header = ASC340StepPrompts.MEMO_HEADER
        processed_text = re.sub(rf'^(#\s*)?{re.escape(asc340_header)}', f'# {asc340_header}', processed_text, flags=re.MULTILINE)
    except ImportError:
...
def convert_memo_to_html(memo_markdown: str, contract_data: Optional[dict] = None) -> str:
    ...
    if contract_data:
        if isinstance(contract_data, dict):
            customer_name = contract_data.get('customer_name', 
                                             contract_data.get('analysis_title', 'Contract Analysis'))
        else:
    ...
    # Determine platform name based on analysis type
    platform_name = "VeritasLogic.ai ASC 606 Analysis Platform"
    if "340" in customer_name or "contract costs" in customer_name.lower():
        platform_name = "VeritasLogic.ai ASC 340-40 Analysis Platform"
...
<head>
    ...
    <title>Analysis: {customer_name}</title>
    {professional_css}
</head>
<body>
    {html_content}
    ...
    <div class="memo-footer">
        <p>Generated by {platform_name} | {current_date}</p>
