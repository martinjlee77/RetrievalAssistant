...
class CleanMemoGenerator:
    ...
    def display_clean_memo(self, memo_content: str, analysis_id: str = None, filename: str = None, customer_name: str = None) -> None:
        ...
        if memo_content and len(memo_content.strip()) > 10:
            ...
            base_filename = f"asc340_memo_{timestamp}"
        ...
        for line in lines:
            ...
            if line.startswith('# '):
                ...
                if in_list:
                    html_lines.append('</ul>')
                    in_list = False
                html_lines.append(f'<h2 style="color: #ffffff; margin: 18px 0 12px 0; border-bottom: 2px solid #e0e0e0; padding-bottom: 5px;">{line[3:]}</h2>')
            elif line.startswith('### '):
                if in_list:
                    html_lines.append('</ul>')
                    in_list = False
                html_lines.append(f'<h3 style="color: #ffffff; margin: 16px 0 10px 0; font-weight: 600;">{line[4:]}</h3>')
